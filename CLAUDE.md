...
- `src/service/mcp/` is the **MCP endpoint**, and is entirely gated behind `[mcp] enabled` (default `false`). With the flag off, `main.rs` registers no route, spawns no task and never constructs the cache, so the nine REST endpoints keep their original behaviour and memory profile.
  ...
  - `admin.rs` — the `/admin` dashboard API, behind its own `admin_token`. It also manages the blocklist, so `main.rs` registers this scope **whenever a token is set, not only when MCP is enabled** — the REST-only instance needs to manage its blocks too. On such an instance `AdminState::mcp_enabled` is false: `/api/state` reports `mcp_enabled: false` with null cache/disk sections and the precache routes refuse with a 400, because reading them would construct a snapshot cache in a process meant to hold none.
...
- `src/static/admin/` is the `/admin` dashboard's own `index.html` / `admin.css` / `admin.js`, served by handlers in `service/mcp/admin.rs` (not by `Files`, so the token check covers the page as well as the API). **External JS only** — the CSP is `script-src 'self'`; `connect-src 'self'` already permits the same-origin `fetch()` calls. Build rows with `textContent`, never `innerHTML`: entry labels and block reasons are operator input. Panels that only make sense with MCP on carry `class="mcp-only"` and are hidden from `payload.mcp_enabled`; the Reload button lives in the blocklist panel because that is the one panel every instance shows.
