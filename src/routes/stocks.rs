...
async fn handler(req: HttpRequest, params: RequestParameters) -> impl Responder {
    ...
    if call_data.is_csv() {
        log_with_ip_uuid(&ip_address, &uuid, format!("Request is csv ('{}')", call_data.data_type.as_deref().unwrap_or("Err.")));
    }
    ...
    // Handling got data
    match data {
        ResponseGet::Stocks(StocksData::CSV(StocksCSV::En(d))) => send_csv(&d.products, "stocks.csv"),
